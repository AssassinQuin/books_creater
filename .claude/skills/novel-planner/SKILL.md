...
## Step 4: Agent 4 — 框架验证器（三视角审查）

### 输入
- Agent 3 的卷级规划
- Agent 1-2 的框架+脉络
- `skill_loader("novel-planner", "engine", "three-perspective-review")`
...
```
读者视角:
- [ ] 开篇有钩子（V1首章有冲突/悬念/异常信号）
- [ ] 信息层级递进（V1-V3建立基础→V4-V7深化→V8-V11颠覆→V12-V14回收）
- [ ] 悬念管理清晰（五种"未知"分布均衡，读者知道在等什么）
- [ ] 角色可识别（主要角色首次出场有明确标识）
- [ ] 无逻辑跳跃（主线每步有前因）
- [ ] 期待感持续（每卷末有钩子引出下卷）
...
作者视角:
- [ ] 起承转合完整（全书有清晰的四段结构）
- [ ] 因果链完整（从起点到终点每步有"因为...所以..."）
- [ ] 伏笔层级清晰（章级/卷级/跨卷/全书/烟雾弹分布合理）
- [ ] 主题一致性（所有卷指向同一核心主题）
- [ ] 节奏控制（高潮/低谷/平台期分布合理）

人物视角:
- [ ] 人物弧光与剧情对齐（每个主要角色的变化节点与卷级事件匹配）
- [ ] 动机充分（主角的目标/恐惧/选择有充分铺垫）
- [ ] 选择必然（关键节点的选择无更简单替代方案）
- [ ] 代价明确（成长/胜利有明确代价）
