...
> **核心情绪**：{一句话} | **章数**：{N}章 | **总字数预估**：{N×3500-4500字}

## 卷级脉络

**因果链**：因为{上卷末/初始条件} → 所以{起点事件} → 逼出{核心选择} → 雪球{连锁} → 到{卷末}
没变的是{遗憾/保留的困境}。

**起承转合**：
- 起（Ch1-{N}·{N}章）：{1句话概述——开篇建立什么}
- 承（Ch{N}-{N}·{N}章）：{1句话概述——冲突升级什么}
- 转（Ch{N}-{N}·{N}章）：{1句话概述——核心转折是什么}
- 合（Ch{N}-{N}·{N}章）：{1句话概述——收束+下卷钩子}

---

## 逐章大纲

### Ch1：{标题}  `D1 上午-傍晚 | {N}场 | {N}字 | 声音:{类型}`
**核心冲突**：{1句话——本章主角面对什么}
**事件链**：{事件A} → {事件B} → {事件C}（因为→所以→逼出）
**关键人物**：{角色A(主)}, {角色B(互动)}, {角色C(背景)}
**伏笔操作**：
- 埋F{?}：{伏笔描述}（通过{动作/对话/物件/环境} — 什么角色在什么场景中自然接触到）
- 深化F{?}：{推进什么}
**世界观展开**：{通过什么角色行动展示世界——拾荒→碎渣经济 / 灵站→制度}
**费笔**：{谁+做了什么日常/世界呼吸}
**章末钩子**：{读者读完想知道的}
**对写作的指导**：
- 开篇从{具体画面/动作/对话}切入
- {哪个场景/哪段对话}是本章情感锚点（写作时重点展开）
- 注意{什么时间线/什么设定连贯性}

---

### Ch2：{标题}  `D2 上午-中午 | {N}场 | {N}字 | 声音:{类型}`
**核心冲突**：{1句话}
**事件链**：{事件A} → {事件B}（因为Ch1的X → 所以Y）
**关键人物**：{角色}
**伏笔操作**：
- ...
**世界观展开**：...
...
| 伏笔ID | 卷内操作 | 位置 | 具体方式 | 计划回收卷 | 重要性 |
|--------|---------|------|---------|-----------|-------|
| F{?} | 埋设 | Ch1 | {场景+方式} | V{N} | 高 |
...
**用户视角**：卷首 + 逐章大纲 = 90% 需要看的内容。想改哪章直接定位到该章块，每章 8-12 行可读完。

**写作视角（novel-chapter-writer）**：
- 加载本文件后，逐章读取 `### Ch{N}` 块
- `核心冲突` + `事件链` → 正文主干
- `伏笔操作` → 融入场景的自然细节（已保证是"自然埋设"非"硬塞"）
- `世界观展开` → 通过角色行动展示而非旁白
- `费笔` → 穿插在主线场景中的世界呼吸
- `章末钩子` → 该章结尾的写作目标
- `对写作的指导` → 开篇方式/情感锚点/注意事项

**修改流程（见🔒检查点A2）**：用户看逐章大纲 → 指出某章要改 → 只改该章块 → 重新展示 → 确认。

##### 硬指标（不达标拒绝存盘）

- 每章块必须包含：核心冲突 + 事件链 + 关键人物 + 伏笔操作 + 章末钩子
- 伏笔操作必须有具体场景化方式（禁止"在Ch{N}提到"，必须描述通过什么动作/物件/环境自然埋设）
- 每章字数预估与事件链的事件数匹配（N事件 × 800-1000字/事件 = 合理字数范围）
- 人物弧光汇总覆盖所有主要角色（主角+≥3配角）
