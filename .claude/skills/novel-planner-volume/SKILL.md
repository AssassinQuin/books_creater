...
2. **每章事件数下限**：≥4个事件（4事件×800字=3200字，保证3000+字底线）。
3. **世界观展开配额**：每章至少1个场景通过事件/互动自然展开世界观（不是堆设定旁白，是通过角色行动展示世界运转方式）。
4. **主角戏份保底**：主角在场场景≥全章场景数的60%。主角不能连续2章无独立事件。
5. **伏笔因果逻辑**：每个伏笔必须有"为什么在这里埋"的因果理由——不是"剧情需要"，而是"这个角色在这个场景中自然会注意到/接触到这个信息"。
...
2. **世界观展开**：每章至少1个场景通过角色行动展开世界观。方式：拾荒流程展示碎渣经济、灵站排队展示制度、市集换物展示物价、修灯展示壁盾遗产。**禁止设定旁白**。
3. **主角戏份保底**：主角在场场景≥60%。主角缺席的场景必须有叙事功能（揭示主角不知道的信息）。
