...
**写前加载**：编排器已传入本章大纲中的 `基调向量` / `世界秩序锚` / `特写配额` 三个字段（来自 chapter-designer 的输出）。场面设计**必须**以这三个字段为约束，不得忽略。
...
🔒 基调调性: {从本章基调向量中选——冷漠克制/突然暴力/短暂温暖/荒诞笑点/兄妹张力}
🔒 写透/粗放: [写透|粗放]（全章只有1个场面可以写透——来自 tone-primer.md §四）
🔒 克制约束: 本场面禁止解释「__」；「__」时刻留白不点破（写出来就失败）
🔒 微动作分配: {角色X}在{情绪时刻Y}做{具体微动作Z}（来自 tone-primer.md §三，空则写「本场面无情绪时刻，不分配」）
...
**🔒 基调字段填写规则**：
- `基调调性`：必须与本章 `基调向量` 一致，场面级可以在叠加范围内微调
- `写透/粗放`：全章只有1个场面是"写透"，其余全标"粗放"——即使那个场面很重要
- `克制约束`：必须写出"禁止解释什么"——如果你不知道这个场面要克制什么，说明你还没想清楚这个场面
- `微动作分配`：从 tone-primer.md §三 的表格中选，不能凭空发明新微动作
...
10. **🔒 世界观基调注入**：每个场面的 `基调调性` / `写透/粗放` / `克制约束` / `微动作分配` 四个字段全部填写，且来自 tone-primer.md 的枚举值
