...
| emotion_type | 核心策略 | 占比规则 | 具体写法示例 |
|---|---|---|----------|
| dialogue | 潜台词+废话+抢话 | 对话≥60% | 「那个...你说呢」+打断+省略号，少用"他说""她问" |
| action | 短句密集+动词驱动 | 动作≥50%，对话≤20% | 「剑气一闪""他冲出去""手掌按住地面"——动词主语，主语鲜明 |
| atmosphere | 五感铺陈+环境暗示情绪 | 环境≥40%，内心≥30% | 视觉+听觉/触觉混合：天空灰得像一块脏布；风声里夹着远处哀嚎；指尖触到的是冷的铁栏杆 |
| psychology | 情绪分层+回忆碎片 | 内心独白≥40% | 三层情绪：当前感受+过去联想+本能反应，用「」区分，不直接描述 |
| daily | 闲聊带设定+毛边细节 | 闲聊+侧面≥60% | 两人闲聊中带过世界观设定：「这批灵石是北境运来的吧？""那倒是，听说上次灵石商会上当街...」 |
| montage | 多片段快切+强画面 | 每片段100-300字 | 用「---」「///」分隔，每段一个独立画面，不连续过渡 |
...
| 避免 | 示例 |
|------|------|
| 每句15-20字均匀 | 「他看着她，她看着你。」「剑光闪过，剑气炸开。」（长短交替） |

**省略过渡：**
| 避免 | 示例 |
|------|------|
| "然而""于是""随后"堆砌 | 手一松，剑落地。他冲了出去。（动作接动作） |

**对话有废话：**
| 避免 | 示例 |
|------|------|
| 完美逻辑对话 | 「那个...你听说了吗？」「啊...我...」+打断+省略号 |

**场景有毛边：**
| 避免 | 示例 |
|------|------|
| 过度规整 | 桌角翘一块皮，桌面有茶渍，窗户缝里塞着一张传单（留真实的不规整） |

**情绪克制：**
| 避免 | 示例 |
|------|------|
| "震惊""难以置信""倒吸凉气" | 指尖在抖。他的瞳孔放大。（动作反应替代情绪词） |

### 多线交织策略

**主线判断：**
1. 查当前卷 main_plotlines → 本章属于哪条线的节点
2. 对比`timeline_query` → 本章在时间线中的位置

**伏笔操作：**
1. 查 `foreshadow_list` → 有计划在附近回收的伏笔吗？
2. 暗线埋设：非暗线章节用日常场景不经意带过
3. 伏笔回收：暗线章节在高潮前3-5章回收

**配角控制：**
1. 配角出场：检查哪些核心配角最近5章没出场
2. 避免堆砌：同章配角不超过3人，每人出场时间≥200字

**场景切换：**
1. 同一章场景切换不超过2次
2. 切换用动作或环境过渡，不用"与此同时""另一边"等显式连接词

**暗线埋设示例：**
「李三点了根烟，烟灰飘向窗外...」（带过北境灵石涨价的设定，不写明）
