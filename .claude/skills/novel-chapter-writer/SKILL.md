...
**字数强制执行机制（新增）：**
```
Step 0: 预估字数 → Step 2: 实时分段检查 → Step 4: 验证

具体执行：
1. Step 0: 读大纲，预估目标字数 2800-3500，写入临时变量 `target_word_count`
2. Step 2: 写作过程中，每写约500字暂停一次，检查当前字数：
   - 如 < 2500 且接近大纲结尾：警告"字数不足，需扩展"
   - 如 > 3500 且大纲未完：警告"大纲未完但字数已超，考虑拆章"
3. Step 4: 写完后验证最终字数，如 < 2500 则拒绝存盘
```
**大纲内容标记（新增）：**
- 写作时明确标注：「[大纲内容]XXX」 vs 「[支线事件]XXX」
- 便于后续检查大纲偏离度
...
**大纲内容标记规范（新增）：**
- 写作时明确标注：`「[大纲内容]XXX」` vs `「[支线事件]XXX」`
- 大纲内容：来自 writing_start 的 outline 字段，必须完成
- 支线事件：大纲外的内容，可灵活扩展但不得偏离主线
- 便于后续检查：区分大纲内容与大纲外支线，确保不偏离主线
### 2.2 设定冲突检测与整改
