...
## Step 1: 战斗设计
- **角色读取**: `character_get(id)` 获取POV角色状态/能力
- **战场环境**: `engine_detail('environment')` → 具体位置+可用元素+5感
- **异兽行为**: 每种≥1独特行为+≥1独特弱点+≥1独特攻击方式
- **战斗弧线**: 铺垫(70%)→转折(5%)→爆发(15%)→代价(10%)
...
## Step 3: 写正文
`engine_detail('action')` 查看动作链5拍。
强制规则：POV锁定 / 动作-反馈链 / 动词>形容词 / 辅助描写≤10字 / 五感反馈 / 实时代价 / 20%细节法则

受伤+物品: 战后 `character_update(status)` + world_upsert
...
## 协作
- 与 chapter-writer 协作：写完章节→有战斗→调本skill→整合
- 与 QA 协作：正文扫描战斗维度→调本skill审计
