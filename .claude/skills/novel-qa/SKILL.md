...
Phase 1: 10维度并行Agent审计（因果逻辑审计参考 `engine_detail('causality')`，因果链断裂=P0）
Phase 2: P0/P1修复（每个问题3方案+代价评估）
Phase 3: 重评，综合≥85通过，最多3轮

参考 `references/outline-review-checklist.md`
...
AI指纹检测：`engine_detail('dialogue')` 查看AI黑名单；`validate_chapter()` 自动扫违禁词
...
## 设定矛盾检查工具
`engine_detail('item')` 查看文章一致性规则
`engine_detail('causality')` 查看因果逻辑审计清单
