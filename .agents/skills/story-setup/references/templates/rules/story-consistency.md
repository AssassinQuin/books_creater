...
`追踪/伏笔.md` 是全书连贯性骨架，commit 时由 `validate-story-commit.sh` 强制校验（违规**阻断**提交）。
表头驱动，兼容两套在用格式，但每条必须满足：

**表头**（含「状态」列即可，校验器自动定位列）：
- v3：`| # | 伏笔 | 埋设卷 | 计划揭示 | 状态 | 分层 | 真实答案 |`
- 旧版：`| ID | 伏笔内容 | 埋设章节 | 预计回收章节 | 状态 | 重要度 |`

**每条伏笔必须**：
- 编号：`F` 开头 + 数字（如 `F1` / `F001`，允许 `F8′` 后缀）
- 状态：取自合法集 `{未埋, 待埋设, 已埋, 已回收}`（「未埋」与「待埋设」语义等价，新旧写法都接受）
- 多表格台账（如 F1-F14 / F15-F22 分区）每个表都要带表头行

**豁免**：预留位行（编号含「预留」、或整行 `—` 占位、或状态为 `—`）不校验。
「忘填状态」（空）不阻断提交，由 SessionStart 的 `detect-story-gaps.sh` advisory 兜底。

**v3 推荐字段**（非强制，旧书可无）：分层 `{深,中,浅}`、真实答案（每条都该有）。

校验实现：`.claude/hooks/lib/foreshadow_schema.py`。
