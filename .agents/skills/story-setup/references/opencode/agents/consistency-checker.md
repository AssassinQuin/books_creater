...
### 伏笔 schema 合规扫描（SC-SCHEMA）
- 检查 `追踪/伏笔.md` 每条伏笔的 schema（commit 时由 validate-story-commit.sh 强制阻断，审查时提前发现）：
  - 编号：`F` 开头 + 数字（如 F1 / F001）
  - 状态：取自合法集 `{未埋, 待埋设, 已埋, 已回收}`
  - 多表格台账（如 F1-F14 / F15-F22 分区）每个表都要带表头行
- schema 定义见 `rules/story-consistency.md` 的「伏笔台账 Schema」段；预留位行豁免
- 违规按 S2 输出（结构错误，不修 commit 会被阻断）
