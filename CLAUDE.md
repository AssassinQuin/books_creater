...
- NOVEL_NAME：`这次不一样`
- 参考作品：无职转生（旅行揭示世界）、权游（残酷不可逆）、全职法师（✓已蒸馏）、将夜（✓已蒸馏）、海贼王（同能力不同开发）、诡秘之主（✓已蒸馏）

### 蒸馏作品检索方式
- 语义检索：`vector_search(novel_name="_参考库", query_text="描述")`
- 关键词：`db_search(novel_name="_参考库", keyword="作品名", top_k=10)`
- 快速查看：`ctx_search(queries=["诡秘之主 {模式名}"], source="ref-distill-诡秘之主")`、`ctx_search(queries=["全职法师 {需求}"], source="ref-patterns-全职法师")`
