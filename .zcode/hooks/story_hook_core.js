...
function trackingCheckpointIssue(book, requireState = false, expectedLastCommitted = null) {
  ...
  if (!fs.existsSync(state)) {
    return requireState
      ? `追踪/_tracking-state.json 缺失；已有正文项目走 story-flow 的旧书迁移（按最后完整章号构造 init 事务重建 追踪/），新书先用 mcp__story-flow__tracking_init 初始化`
      : null
  }
  let document
  try {
    ...
    return `追踪/_tracking-state.json 无法解析；停止写正文并用 tracking_commit 事务重建，不能猜测或手补状态`
  }
  if (!document || typeof document !== "object" || Array.isArray(document) || document.schema_version !== 4) {
    return `追踪/_tracking-state.json 不是当前 schema_version=4；停止写正文并用 tracking_commit 事务重建，不保留旧结构兼容路径`
  }
  if (!Number.isInteger(document.state_revision)) {
    return `追踪/_tracking-state.json 缺少整数 state_revision；停止写正文并用 tracking_commit 事务重建`
  }
  ...
  if (expectedLastCommitted !== null) {
    if (!Number.isInteger(document.last_committed_chapter)) {
      return `追踪/_tracking-state.json 缺少整数 last_committed_chapter；停止写正文并用 tracking_commit 事务重建`
    }
...
function proseBlockReason(root, absolute) {
  ...
  if (base === "正文.md") {
    ...
    if (!fs.existsSync(path.join(book, "小节大纲.md"))) {
      return `⛔ 写正文被拦截：${safeRelative(root, absolute)} 缺少同目录 小节大纲.md。先按 novel-setup/character 流程完成「小节大纲.md」再写正文。`
    }
    return null
  }
  ...
  // 让核心守卫 fail open。
  // 旧书导入在复制既有正文、尚未执行 tracking init 的窗口可以写；一旦 state 存在，
  ...
  if (!exists) {
    ...
    if (!found) {
      return `⛔ 写正文被拦截：第 ${chapter} 章缺少细纲（${safeRelative(root, outlineDir)}/细纲_第${chapter}章.md）。先按 novel-planner 单章补纲流程重建细纲再写正文。`
    }
  }
